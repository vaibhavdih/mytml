...
def select_parent_by_area(
    parent_candidates: List[DiagramComponent],
) -> DiagramComponent:
    parent_candidates.sort(key=calculate_area)
    return parent_candidates[0]


def select_parent_by_centroid(
    child: DiagramComponent, parent_candidates: List[DiagramComponent]
) -> DiagramComponent:
    ...
    for parent_candidate in parent_candidates:
        distance = abs(
            parent_candidate.representation.centroid.distance(child_centroid)
        )
...
def select_parent(
    child: DiagramComponent, potential_parents: List[DiagramComponent]
) -> DiagramComponent:
    if len(potential_parents) == 1:
        return potential_parents[0]

    component_parents = list(
        filter(
            lambda p: p.origin == DiagramComponentOrigin.SIMPLE_COMPONENT,
            potential_parents,
        )
    )
...
def is_contained(
    parent_candidate: DiagramComponent, child_candidate: DiagramComponent
) -> bool:
    return (
        parent_candidate.id != child_candidate.id
        and parent_candidate.representation.contains(child_candidate.representation)
    )
...
class ParentCalculator:
    ...
    def calculate_parent(
        self, parent_candidates: List[DiagramComponent]
    ) -> DiagramComponent:
        potential_parents = []
        for parent_candidate in parent_candidates:
